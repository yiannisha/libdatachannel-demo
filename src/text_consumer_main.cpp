...
demo::WebSocketSignalTransportConfig parseOptions(int argc, char** argv) {
  if (argc < 2) {
    throw std::invalid_argument(
        "usage: text_consumer (--listen <port> [bind_address] | --connect <host> <port> | <ws-url> | <host> <port>)");
  }

  const std::string mode = argv[1];
  if (mode == "--listen") {
    if (argc < 3 || argc > 4) {
      throw std::invalid_argument("usage: text_consumer --listen <port> [bind_address]");
    }
  ...
  if (mode == "--connect") {
    ...
    throw std::invalid_argument(
        "usage: text_consumer --connect <host> <port> [--retry-ms <ms>] or text_consumer --connect <ws-url> [--retry-ms <ms>]");
  }
  ...
  throw std::invalid_argument(
      "usage: text_consumer (--listen <port> [bind_address] | --connect <host> <port> [--retry-ms <ms>] | --connect <ws-url> [--retry-ms <ms>] | <ws-url> | <host> <port>)");
}
...
int main(int argc, char** argv) {
  try {
    ...
    demo::TextConsumer consumer(parseOptions(argc, argv));
    if (consumer.isSignalingServer()) {
      std::cout << "text consumer websocket server listening on "
                << consumer.signalingEndpoint() << '\n';
    } else {
      std::cout << "text consumer websocket client targeting "
                << consumer.signalingEndpoint() << '\n';
    }
    consumer.wait();
    return EXIT_SUCCESS;
